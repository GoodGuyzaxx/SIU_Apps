...
<div class="container">
    ...
    <div class="signature-section">
        <div class="signature-box">
            ...
            @php
                $ttdSrc = null;
                if (isset($kaprodi) && $kaprodi->ttd) {
                    $pathsToTry = [
                        storage_path('app/private/' . $kaprodi->ttd),
                        storage_path('app/public/' . $kaprodi->ttd),
                        storage_path('app/' . $kaprodi->ttd),
                    ];

                    foreach ($pathsToTry as $path) {
                        if (file_exists($path)) {
                            $ttdData = base64_encode(file_get_contents($path));
                            $mime = mime_content_type($path) ?: 'image/png';
                            $ttdSrc = 'data:' . $mime . ';base64,' . $ttdData;
                            break;
                        }
                    }
                }
            @endphp
            @if($ttdSrc)
                <img style="max-width: 70%" src="{{ $ttdSrc }}"/>
            @else
                <br><br><br><br>
            @endif
